...
class MoleculeVocab:
    ...
    def __init__(self, use_small_mol=False):
        ...
        specials = [(self.UNK, 'UNK')]
        aas = const.aas

        self.atom_pad, self.atom_mask = 'pad', 'msk'
        self.atom_pos_pad, self.atom_pos_mask = 'pad', 'msk'

        # block level vocab
        self.idx2block = aas + specials
        ...
        # atom level vocab
        self.idx2atom = [self.atom_pad, self.atom_mask]
        self.idx2atom_pos = [self.atom_pos_pad, self.atom_pos_mask, '', 'A', 'B', 'G', 'D', 'E', 'Z', 'H', 'XT', 'P']
...
class Block:
    ...
    def from_tuple(self, data):
        return Block(
            ...
            id=data[2] if len(data) == 3 else None
        )
