...
class ConfidenceDataModule(pl.LightningDataModule):
    def __init__(
        self,
        datasets: DictConfig,
        num_workers: DictConfig,
    ...
    def conf_collate_fn(self, batch):
        try:
            return [Batch.from_data_list(s) for s in zip(*batch)]
        except:
            pdb.set_trace()


    def train_dataloader(self, shuffle = True) -> TDataLoader:
        return TDataLoader(
            self.train_dataset,
            shuffle=shuffle,
            batch_size=self.batch_size.train,
            num_workers=self.num_workers.train,
            worker_init_fn=worker_init_fn,
            collate_fn=lambda batch:self.conf_collate_fn(batch)
        )

    def val_dataloader(self) -> Sequence[TDataLoader]:
        return [
            TDataLoader(
                dataset,
                shuffle=False,
    ...
    def test_dataloader(self) -> Sequence[TDataLoader]:
        return [
            DataLoader(
                dataset,
                shuffle=False,
